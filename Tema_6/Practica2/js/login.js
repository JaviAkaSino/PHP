const DIR_SERV="http://localhost/PHP/Tema_6/Practica2/Ejercicio1_key/servicios_rest_ejer1";
const TIEMPO_SESION_MINUTOS=2


$(document).ready(function(){
   
    if(localStorage.ultimo_acceso && localStorage.api_session)
    {
...
{
    $('#errores').html("");
    var html_output="<div id='saludo'>Bienvenido <strong>"+datos_usu_log["usuario"]+"</strong> - <button onclick='cerrar_sesion();' class='enlace'>Salir</button></div>";
...
{
    $('#errores').html("");
    var html_output="<form onsubmit='event.preventDefault();hacer_login();'>";
    html_output+="<p><label for='usuario'>Usuario: </label><input type='text' id='usuario' required/></p>";
    html_output+="<p><label for='clave'>Contraseña: </label><input onkeyup='cifrar_clave();' type='password' id='clave' required/><input id='clave2' type='hidden'/></p>";
    html_output+="<p><button>Login</button></p>";
    html_output+="<p id='error_login'>"+error+"</p>";
    html_output+="</form>";

    $('#principal').html(html_output);
}

function cifrar_clave()
{

       $("#clave2").val(md5($("#clave").val()));
